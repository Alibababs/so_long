...
/*   Updated: 2024/07/29 17:16:36 by pbailly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_background(t_data *data)
{
	...
	data->background = mlx_xpm_file_to_image(data->mlx, path_img, &img_width,
			&img_height);
	if (!data->background)
		error_msg("Error\nImage of the background couldn't load", data);
	y = 0;
	while (y < data->size_y)
	{
		x = 0;
...
void	init_obj(t_data *data, char *path)
{
	...
	if (!data->obj)
		error_msg("Error\nImage of the map couldn't load", data);
...
void	init_player(t_data *data, char *path)
{
	...
	data->player = mlx_xpm_file_to_image(data->mlx, path, &img_width,
			&img_height);
	if (!data->player)
		error_msg("Error\nImage of the player couldn't load", data);
...
void	init_text_box(t_data *data)
{
	...
	if (!data->text_box)
		error_msg("Error\nImage of the text_box couldn't load", data);
